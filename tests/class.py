...
class VerminClassMemberTests(VerminTest):
  ...
  def test_RawConfigParser_of_ConfigParser(self):
    self.assertOnlyIn((2, 3), detect("from ConfigParser import RawConfigParser"))

  def test_SafeConfigParser_of_ConfigParser(self):
    self.assertOnlyIn((2, 3), detect("from ConfigParser import SafeConfigParser"))

  def test_LifoQueue_of_Queue(self):
    self.assertOnlyIn((2, 6), detect("from Queue import LifoQueue"))

  def test_PriorityQueue_of_Queue(self):
    self.assertOnlyIn((2, 6), detect("from Queue import PriorityQueue"))
  ...
  def test_HTMLCalendar_of_calendar(self):
    self.assertOnlyIn(((2, 5), (3, 0)), detect("from calendar import HTMLCalendar"))

  def test_LocaleHTMLCalendar_of_calendar(self):
    self.assertOnlyIn(((2, 5), (3, 0)), detect("from calendar import LocaleHTMLCalendar"))

  def test_LocaleTextCalendar_of_calendar(self):
    self.assertOnlyIn(((2, 5), (3, 0)), detect("from calendar import LocaleTextCalendar"))

  def test_TextCalendar_of_calendar(self):
    self.assertOnlyIn(((2, 5), (3, 0)), detect("from calendar import TextCalendar"))

  def test_IncrementalEncoder_of_codecs(self):
    self.assertOnlyIn(((2, 5), (3, 0)), detect("from codecs import IncrementalEncoder"))

  def test_NNTP_SSL_of_nntplib(self):
    self.assertOnlyIn((3, 2), detect("from nntplib import NNTP_SSL"))

  def test_DirEntry_of_os(self):
    self.assertOnlyIn((3, 5), detect("from os import DirEntry"))

  def test_PathLike_of_os(self):
    self.assertOnlyIn((3, 6), detect("from os import PathLike"))

  def test_sched_param_of_os(self):
    self.assertOnlyIn((3, 3), detect("from os import sched_param"))
  ...
  def test_PickleBuffer_of_pickle(self):
    self.assertOnlyIn((3, 8), detect("from pickle import PickleBuffer"))

  def test_Popen4_of_popen2(self):
    self.assertOnlyIn((2, 0), detect("from popen2 import Popen4"))

  def test_POP3_SSL_of_poplib(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from poplib import POP3_SSL"))

  def test_Stats_of_pstats(self):
    self.assertOnlyIn(((2, 3), (3, 0)), detect("from pstats import Stats"))

  def test_PycInvalidationMode_of_py_compile(self):
    self.assertOnlyIn((3, 7), detect("from py_compile import PycInvalidationMode"))

  def test_SimpleQueue_of_queue(self):
    self.assertOnlyIn((3, 7), detect("from queue import SimpleQueue"))

  def test_SystemRandom_of_random(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from random import SystemRandom"))

  def test_DevpollSelector_of_selectors(self):
    self.assertOnlyIn((3, 5), detect("from selectors import DevpollSelector"))
  ...
  def test_IsolatedAsyncioTestCase_of_unittest(self):
    self.assertOnlyIn((3, 8), detect("from unittest import IsolatedAsyncioTestCase"))

  def test_TextTestResult_of_unittest(self):
    self.assertOnlyIn(((2, 7), (3, 2)), detect("from unittest import TextTestResult"))

  def test_AsyncMock_of_unittest_mock(self):
    self.assertOnlyIn((3, 8), detect("from unittest.mock import AsyncMock"))

  def test_DefragResult_of_urllib_parse(self):
    self.assertOnlyIn((3, 2), detect("from urllib.parse import DefragResult"))

  def test_DefragResultBytes_of_urllib_parse(self):
    self.assertOnlyIn((3, 2), detect("from urllib.parse import DefragResultBytes"))

  def test_ParseResultBytes_of_urllib_parse(self):
    self.assertOnlyIn((3, 2), detect("from urllib.parse import ParseResultBytes"))

  def test_SplitResultBytes_of_urllib_parse(self):
    self.assertOnlyIn((3, 2), detect("from urllib.parse import SplitResultBytes"))

  def test_DataHandler_of_urllib_request(self):
    self.assertOnlyIn((3, 4), detect("from urllib.request import DataHandler"))

  def test_HTTPPasswordMgrWithPriorAuth_of_urllib_request(self):
    self.assertOnlyIn((3, 5), detect("from urllib.request import HTTPPasswordMgrWithPriorAuth"))

  def test_HTTPCookieProcessor_of_urllib2(self):
    self.assertOnlyIn((2, 4), detect("from urllib2 import HTTPCookieProcessor"))

  def test_HTTPErrorProcessor_of_urllib2(self):
    self.assertOnlyIn((2, 4), detect("from urllib2 import HTTPErrorProcessor"))

  def test_SafeUUID_of_uuid(self):
    self.assertOnlyIn((3, 7), detect("from uuid import SafeUUID"))
  ...
  def test_Deque_of_typing(self):
    self.assertOnlyIn((3, 5), detect("from typing import Deque"))
    self.assertTrue(self.config.add_backport("typing"))
    self.assertOnlyIn(((2, 7), (3, 5)), detect("from typing import Deque"))

  def test_ContextManager_of_typing(self):
    self.assertOnlyIn((3, 5), detect("from typing import ContextManager"))
    self.assertTrue(self.config.add_backport("typing"))
    self.assertOnlyIn(((2, 7), (3, 5)), detect("from typing import ContextManager"))

  def test_Coroutine_of_typing(self):
    self.assertOnlyIn((3, 5), detect("from typing import Coroutine"))
    self.assertTrue(self.config.add_backport("typing"))
    self.assertOnlyIn(((2, 7), (3, 5)), detect("from typing import Coroutine"))

  def test_DefaultDict_of_typing(self):
    self.assertOnlyIn((3, 5), detect("from typing import DefaultDict"))
    self.assertTrue(self.config.add_backport("typing"))
    self.assertOnlyIn(((2, 7), (3, 5)), detect("from typing import DefaultDict"))

  def test_OrderedDict_of_typing(self):
    self.assertOnlyIn((3, 7), detect("from typing import OrderedDict"))
    self.assertTrue(self.config.add_backport("typing"))
    self.assertOnlyIn(((2, 7), (3, 7)), detect("from typing import OrderedDict"))
  ...
  def test_Counter_of_typing(self):
    self.assertOnlyIn((3, 5), detect("from typing import Counter"))
    self.assertTrue(self.config.add_backport("typing"))
    self.assertOnlyIn(((2, 7), (3, 5)), detect("from typing import Counter"))
  ...
  def test_Text_of_typing(self):
    self.assertOnlyIn((3, 5), detect("from typing import Text"))
    self.assertTrue(self.config.add_backport("typing"))
    self.assertOnlyIn(((2, 7), (3, 5)), detect("from typing import Text"))
  ...
  def test_MappingProxyType_of_types(self):
    self.assertOnlyIn((3, 3), detect("from types import MappingProxyType"))

  def test_SimpleNamespace_of_types(self):
    self.assertOnlyIn((3, 3), detect("from types import SimpleNamespace"))
  ...
  def test_NullHandler_of_logging(self):
    self.assertOnlyIn(((2, 7), (3, 1)), detect("from logging import NullHandler"))

  def test_QueueHandler_of_logging_handlers(self):
    self.assertOnlyIn((3, 2), detect("from logging.handlers import QueueHandler"))

  def test_QueueListener_of_logging_handlers(self):
    self.assertOnlyIn((3, 2), detect("from logging.handlers import QueueListener"))

  def test_WatchedFileHandler_of_logging_handlers(self):
    self.assertOnlyIn(((2, 6), (3, 0)), detect("from logging.handlers import WatchedFileHandler"))
  ...
  def test_MemoryBIO_of_ssl(self):
    self.assertOnlyIn((3, 5), detect("from ssl import MemoryBIO"))

  def test_Options_of_ssl(self):
    self.assertOnlyIn((3, 6), detect("from ssl import Options"))
  ...
  def test_SSLObject_of_ssl(self):
    self.assertOnlyIn((3, 5), detect("from ssl import SSLObject"))
  ...
  def test_Formatter_of_string(self):
    self.assertOnlyIn(((2, 6), (3, 0)), detect("from string import Formatter"))

  def test_Template_of_string(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from string import Template"))

  def test_Struct_of_struct(self):
    self.assertOnlyIn(((2, 5), (3, 0)), detect("from struct import Struct"))
  ...
  def test_EnvironmentVarGuard_of_test_support(self):
    self.assertOnlyIn(((2, 6), (3, 0)), detect("from test.support import EnvironmentVarGuard"))

  def test_TransientResource_of_test_support(self):
    self.assertOnlyIn(((2, 6), (3, 0)), detect("from test.support import TransientResource"))

  def test_WarningsRecorder_of_test_support(self):
    self.assertOnlyIn(((2, 6), (3, 0)), detect("from test.support import WarningsRecorder"))

  def test_Barrier_of_threading(self):
    self.assertOnlyIn((3, 2), detect("from threading import Barrier"))

  def test_local_of_threading(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from threading import local"))

  def test_struct_time_of_time(self):
    self.assertOnlyIn(((2, 2), (3, 0)), detect("from time import struct_time"))

  def test_FrameSummary_of_traceback(self):
    self.assertOnlyIn((3, 5), detect("from traceback import FrameSummary"))

  def test_StackSummary_of_traceback(self):
    self.assertOnlyIn((3, 5), detect("from traceback import StackSummary"))

  def test_TracebackException_of_traceback(self):
    self.assertOnlyIn((3, 5), detect("from traceback import TracebackException"))
  ...
  def test_EmailPolicy_of_email_policy(self):
    self.assertOnlyIn((3, 3), detect("from email.policy import EmailPolicy"))
  ...
  def test_Flag_of_enum(self):
    self.assertOnlyIn((3, 6), detect("from enum import Flag"))

  def test_IntFlag_of_enum(self):
    self.assertOnlyIn((3, 6), detect("from enum import IntFlag"))

  def test_auto_of_enum(self):
    self.assertOnlyIn((3, 6), detect("from enum import auto"))

  def test_FTP_TLS_of_ftplib(self):
    self.assertOnlyIn(((2, 7), (3, 2)), detect("from ftplib import FTP_TLS"))

  def test_partialmethod_of_functools(self):
    self.assertOnlyIn((3, 4), detect("from functools import partialmethod"))

  def test_HTTPStatus_of_http(self):
    self.assertOnlyIn((3, 5), detect("from http import HTTPStatus"))

  def test_ThreadingHTTPServer_of_http_server(self):
    self.assertOnlyIn((3, 7), detect("from http.server import ThreadingHTTPServer"))

  def test_HTTPConnection_of_httplib(self):
    self.assertOnlyIn((2, 0), detect("from httplib import HTTPConnection"))

  def test_HTTPResponse_of_httplib(self):
    self.assertOnlyIn((2, 0), detect("from httplib import HTTPResponse"))

  def test_HTTPSConnection_of_httplib(self):
    self.assertOnlyIn((2, 0), detect("from httplib import HTTPSConnection"))

  def test_IMAP4_stream_of_imaplib(self):
    self.assertOnlyIn(((2, 3), (3, 0)), detect("from imaplib import IMAP4_stream"))

  def test_NullImporter_of_imp(self):
    self.assertOnlyIn(((2, 5), (3, 0)), detect("from imp import NullImporter"))

  def test_FileLoader_of_importlib_abc(self):
    self.assertOnlyIn((3, 3), detect("from importlib.abc import FileLoader"))

  def test_MetaPathFinder_of_importlib_abc(self):
    self.assertOnlyIn((3, 3), detect("from importlib.abc import MetaPathFinder"))

  def test_PathEntryFinder_of_importlib_abc(self):
    self.assertOnlyIn((3, 3), detect("from importlib.abc import PathEntryFinder"))
  ...
  def test_DocTest_of_doctest(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from doctest import DocTest"))

  def test_DocTestFinder_of_doctest(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from doctest import DocTestFinder"))

  def test_DocTestParser_of_doctest(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from doctest import DocTestParser"))

  def test_DocTestRunner_of_doctest(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from doctest import DocTestRunner"))

  def test_Example_of_doctest(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from doctest import Example"))

  def test_OutputChecker_of_doctest(self):
    self.assertOnlyIn(((2, 4), (3, 0)), detect("from doctest import OutputChecker"))
  ...
  def test_Path_of_zipfile(self):
    self.assertOnlyIn((3, 8), detect("from zipfile import Path"))
