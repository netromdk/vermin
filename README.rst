...
matches against internal dictionaries with 915 rules divided into 120 modules, 634
classes/functions/constants members of modules, 152 kwargs of functions, 4 strftime directives, 2
array typecodes, and 3 codecs error handler names. Including looking for v2/v3 ``print expr`` and
``print(expr)``, ``long``, f-strings, coroutines (``async`` and ``await``), boolean constants,
``"..".format(..)``, imports (``import X``, ``from X import Y``, ``from X import *``), function
calls wrt. name and kwargs, ``strftime`` + ``strptime`` directives used, and function, variable
annotations, array typecodes, codecs error handler names. It tries to detect and ignore user-defined
